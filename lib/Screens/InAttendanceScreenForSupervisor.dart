...
class InAttendanceForSupervisorScreen extends StatefulWidget {
  ...
  State<InAttendanceForSupervisorScreen> createState() =>
      _InAttendanceForSupervisorScreenState();
}

class _InAttendanceForSupervisorScreenState
    extends State<InAttendanceForSupervisorScreen>
    ...
    try {
      ...
      if (responseData is Map<String, dynamic>) {
        ...
        if (status == 'TRUE') {
          ...
          _showNullValueError(
              "getDistanceData Api : Failed to fetch distance, status = $status");
        }
      } else {
        throw Exception("getDistanceData Api : Invalid response format.");
      }
    } catch (e) {
      print("getDistanceData Error: $e");
      _showNullValueError("getDistanceData Api Exception: ${e.toString()}");
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
  ...
  Future<bool> getUserFaceData() async {
    try {
      ...
      String? base64Image = prefs
          .getString('super_user_face_data')
